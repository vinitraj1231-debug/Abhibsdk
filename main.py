...
def stylish(text):
    if not text: return ""
    # Use Monospace for digits and small caps for letters
    mapping = {
    ...
    def _rep(m):
        t = m.group(0)
        if (t.startswith('{') and t.endswith('}')) or t.startswith('/') or (t.startswith('<') and t.endswith('>')):
            return t
        return "".join(mapping.get(c, c) for c in t)

    # Improved regex to avoid mapping HTML tags
...
async def save_meta(client, meta: dict) -> bool:
    ...
    try:
        ...
        if main_client and main_client != client:
            try:
                await main_client.send_message(DB_CHANNEL, txt)
                return True
            except: pass
        if GLOBAL_USERBOT:
            try:
                await GLOBAL_USERBOT.send_message(DB_CHANNEL, txt)
                return True
            except: pass
    return False
...
async def rebuild_phase2(userbot, upd_fn, since_date=None, latest_id: int = 0) -> dict:
    ...
    while offset_id > 1 and not stopped:
        ...
        await upd_fn(
            ...
            f" Config/Admins: `{stats['config'] + stats['admins']}`"
        )
        await asyncio.sleep(0.1)
...
async def smart_rebuild(status_msg=None) -> dict:
    ...
    if not SESSION_STRING:
        await upd(
            ...
            "**How to fix:**\n"
            "1⃣ Run `python generate_session.py` locally.\n"
            "2⃣ Copy the session string.\n"
            "3⃣ Add it to your Environment Variables as `SESSION_STRING`.\n"
            "4⃣ Restart the bot.\n\n"
...
def kb_start(bot_id, user_id):
    ...
    rows = [
        ...
        [InlineKeyboardButton(stylish("ABOUT"), callback_data="about_menu"),
         InlineKeyboardButton(stylish("HELP"),  callback_data="help_categories")]
    ]
    ...
    async def rebuild_cmd(client, message):
        ...
        if not SESSION_STRING:
            return await message.reply(
                ...
                "1⃣ `pip install pyrogram TgCrypto`\n"
                "2⃣ `python generate_session.py`\n"
                "3⃣ Set `SESSION_STRING` env var\n"
                "4⃣ Restart → `/rebuild` will work"
            )
    ...
    async def start_handler(client, message):
        ...
        if bi and bi.get("verify_link") and not is_admin(uid) and uid != bi.get("owner_id"):
            if not deep.startswith("verify_"):
                ...
                return await message.reply(
                    ...
                    f"1⃣ Click the **Verification** button below.\n"
                    f"2⃣ Complete the process in the other bot.\n"
                    f"3⃣ Come back here and click `/start` again.",
        ...
        if is_new and bi and bi.get("owner_id") == uid:
            await message.reply(
                ...
                f"1⃣ `/setlog -100xxxx` - Set a log channel to see uploads.\n"
                f"2⃣ `/setchannel -100xxxx` - Connect your channel for the `/start join` link.\n"
                f"3⃣ `/setmode requested` - If you want users to send join requests.\n"
                f"4⃣ `/setwelcome` - Customize this message.\n\n"
    ...
    async def broadcast_cmd(client, message):
        ...
        if not message.reply_to_message:
            ...
            return await message.reply(
                ...
                f"**HOW TO USE:**\n"
                f"1⃣ Reply to any message with `/broadcast`.\n"
                f"2⃣ You can optionally add a button like this:\n"
    ...
    async def clone_cmd(client, message):
        ...
        if len(message.command)<2:
            ...
            return await message.reply(
                ...
                f"1⃣ Go to @BotFather and create a `/newbot`.\n"
                f"2⃣ Copy the **API TOKEN** they give you.\n"
                f"3⃣ Send it here: `/clone YOUR_TOKEN`.\n\n"
        ...
        try:
            ...
            if na:
                ...
                await sm.edit(
                    ...
                    f"1⃣ Open your new bot: @{me.username}\n"
                    f"2⃣ Send `/start` to activate it.\n"
                    f"3⃣ Use `/setlog -100xxxx` to set a log channel.\n"
                    f"4⃣ Use `/setchannel` to connect your main channel.\n\n"
    ...
    async def cb_handler(client, cb):
        ...
        # ── Dual post callbacks ───────────────────────────────────
            ...
            await cb.message.edit(
                ...
                "1⃣ `/dualpost Title` — Start a new session.\n"
                "2⃣ Send files for **FREE** users (Stage 1).\n"
                "3⃣ `/dpremium` — Switch to premium stage.\n"
                "4⃣ Send files for **PREMIUM** users (Stage 2).\n"
                "5⃣ `/dpdone` — Finalize and get your link.\n\n"
        ...
        # ── Admin callbacks ───────────────────────────────────────
            ...
            sess    = "ACTIVATED"if SESSION_STRING else " NOT SET (Data at risk!)"
            active_tokens = sum(1 for v in SHORTENER_TOKENS.values()
                                if not v["used"] and time.time() < v["expires_at"])
            ...
            bi  = get_bot_info(bot_id)
            text = (bi.get("custom_welcome") if bi else None)
            if not text:
                text = (
                    "<blockquote>ʜᴇʟʟᴏ {name}\n\n"
                    "ɪ ᴀᴍ ғɪʟᴇ sᴛᴏʀᴇ ʙᴏᴛ, ɪ ᴄᴀɴ sᴛᴏʀᴇ ᴘʀɪᴠᴀᴛᴇ ғɪʟᴇs ɪɴ sᴘᴇᴄɪғɪᴇᴅ ᴄʜᴀɴɴᴇʟ ᴀɴᴅ ᴏᴛʜᴇʀ ᴜsᴇʀs ᴄᴀɴ ᴀᴄᴄᴇss ɪᴛ ғʀᴏᴍ sᴘᴇᴄɪᴀʟ ʟɪɴᴋ.\n\n"
            ...
            text = (
                "<blockquote>ABOUT THIS BOT\n\n"
                "Developers: @zolvid\n"
                "Support Group: @zolvid_support\n"
                "Channel: @zolvid_channel</blockquote>"
            )
            await cb.message.edit(stylish(text), reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(stylish("BACK"), callback_data="back_to_start")]
            ]))
            await cb.answer()

        elif data == "help_categories":
            btns = [
                [InlineKeyboardButton(stylish("GENERAL"), callback_data="help_gen"),
                 InlineKeyboardButton(stylish("FILE MGMT"), callback_data="help_file")],
                [InlineKeyboardButton(stylish("ADVANCED"), callback_data="help_adv"),
                 InlineKeyboardButton(stylish("ADMIN"), callback_data="help_adm")],
                [InlineKeyboardButton(stylish("BACK"), callback_data="back_to_start")]
            ]
            await cb.message.edit(stylish("<blockquote>HELP CATEGORIES\n\nSelect a category to see details.</blockquote>"), reply_markup=InlineKeyboardMarkup(btns))
            await cb.answer()
            ...
            text = (
                ...
                "/premium - Premium info</blockquote>"
            )
            await cb.message.edit(stylish(text), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(stylish("BACK"), callback_data="help_categories")]]))
            await cb.answer()

        elif data == "help_file":
            text = (
                "<blockquote>FILE MANAGEMENT\n\n"
                "/batch - Start batch mode\n"
                "/done - Finish session\n"
                "/cancel - Cancel action\n"
                "/listfiles - List files\n"
                "/myduals - Manage dual posts</blockquote>"
            )
            await cb.message.edit(stylish(text), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(stylish("BACK"), callback_data="help_categories")]]))
            await cb.answer()
            ...
            text = (
                "<blockquote>ADVANCED FEATURES\n\n"
                "/clone - Create own bot\n"
                "/dualpost - Create dual-tier post\n"
                "/protect - Protect channel link\n"
                "/mybots - List cloned bots</blockquote>"
            )
            await cb.message.edit(stylish(text), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(stylish("BACK"), callback_data="help_categories")]]))
            await cb.answer()
            ...
            text = (
                "<blockquote>ADMIN TOOLS\n\n"
                "/admin - Admin Panel\n"
                "/setfs - Force Sub\n"
                "/setwelcome - Welcome msg\n"
                "/setlog - Log channel</blockquote>"
            )
            await cb.message.edit(stylish(text), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(stylish("BACK"), callback_data="help_categories")]]))
            await cb.answer()
...
async def main():
    ...
    critical_dbs = [BOTS_DB, FILES_DB, USERS_DB, ADMINS_DB, CONFIG_DB]
    needs_restore = False
    ...
    if needs_restore:
        logger.warning(" Critical database missing or empty! Attempting auto-restore from DB_CHANNEL...")
        if SESSION_STRING:
            try:
                # Wait a bit for everything to stabilize
                await asyncio.sleep(2)

                # We need a temporary client to perform rebuild if no clients are active
                async def temp_upd(text): logger.info(f"Rebuild: {text}")

                async with Client(
                    "auto_rebuild_task", api_id=API_ID, api_hash=API_HASH,
                    session_string=SESSION_STRING, in_memory=True
                ) as userbot:
                    logger.info(" Auto-restore: Phase 1 (Files)...")
                    restored_phase1 = await rebuild_phase1(userbot, temp_upd)

                    backup_date = None
                    if restored_phase1:
                        dates = [d for d in restored_phase1.values() if d]
                        if dates: backup_date = max(dates)

                    logger.info(" Auto-restore: Phase 2 (Metadata)...")
                    stats2 = await rebuild_phase2(userbot, temp_upd, since_date=backup_date)

                    logger.info(f" Auto-restore complete! Phase 1: {len(restored_phase1)} files. Phase 2: {stats2['files']} files, {stats2['bots']} bots.")

                # After successful auto-restore, we MUST restart to initialize all bots properly
                logger.info(" Restarting to apply restored data...")
                os.execl(sys.executable, sys.executable, *sys.argv)
            except Exception as e:
                logger.error(f"Auto-restore failed: {e}")
        else:
            logger.warning(" SESSION_STRING missing! Cannot auto-restore from channel. Please ensure SESSION_STRING is set for disaster recovery.")
